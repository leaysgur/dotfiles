// Merged cursor shader for Ghostty.
// Combines two effects from https://github.com/qwerasd205/ghostty-cursor-shaders
//   1. cursor_warp.glsl  (smear/warp trail)
//   2. ripple_cursor.glsl (mode-change ripple ring)
// They were originally two chained `custom-shader` passes. Here both `mainImage`
// bodies are merged into one, applied in the same order (warp first, ripple on top).
...
const float WARP_DURATION = 0.2; // total animation time
...
const float WARP_BLUR = 1.0; // blur size in pixels (for antialiasing)
...
// Applies the warp/smear trail on top of `color`, returns the new color.
vec4 applyWarp(vec4 color, vec2 vu, vec4 currentCursor, vec4 previousCursor, vec2 centerCC) {
    ...
    if (lineLength > minDist && baseProgress < WARP_DURATION - 0.001) {
        ...
        if (WARP_BLUR < 2.5) {
          ...
          float effectiveBlur = mix(0.0, WARP_BLUR, isDiagonal);
        }
...
// CONFIGURATION
const float RIPPLE_DURATION = 0.15;               // How long the ripple animates (seconds)
...
vec4 RIPPLE_COLOR = vec4(0.35, 0.36, 0.44, 1.0); // change to iCurrentCursorColor for your cursor's color
const float RIPPLE_BLUR = 3.0;                    // Blur level in pixels
...
// EaseOutCirc (ripple)
float rippleEaseOutCirc(float t) {
    return sqrt(1.0 - pow(t - 1.0, 2.0));
}

// Applies the mode-change ripple on top of `color`, returns the new color.
vec4 applyRipple(vec4 color, vec2 vu, vec4 currentCursor, vec4 previousCursor, vec2 centerCC) {
    ...
    if (isModeChange > 0.0 && isAnimating > 0.0) {
        float easedProgress = rippleEaseOutCirc(rippleProgress);
        ...
        float antiAliasSize = normalize(vec2(RIPPLE_BLUR, RIPPLE_BLUR), 0.0).x;
        float ripple = (1.0 - smoothstep(-antiAliasSize, antiAliasSize, sdfRing)) * fade;

        // Apply ripple effect
        color = mix(color, RIPPLE_COLOR, ripple * RIPPLE_COLOR.a);
    }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Shared normalization & setup (-1 to 1 coords)
    ...
    // Apply effects in the original chain order: warp first, ripple on top.
    fragColor = applyWarp(fragColor, vu, currentCursor, previousCursor, centerCC);
    fragColor = applyRipple(fragColor, vu, currentCursor, previousCursor, centerCC);
}
